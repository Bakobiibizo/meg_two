...
def meeting_minutes(
    transcription: str,
    audio_file_name: str,
    roles_and_extracted_info: tuple,
    timestamp: str,
) -> dict:
    try:
        if not transcription or not audio_file_name:
            raise ValueError(
                "Both 'transcription' and 'audio_file_name' must be provided and not empty."
            )
        ...
        for role, extracted_info in roles_and_extracted_info:
            minutes[role] = extracted_info
